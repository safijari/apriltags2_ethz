from aprilgrid.tagFamilies import t36h11
...
class AprilGrid(object):
    def __init__(
        self, rows, columns, size, spacing, family=t36h11, shift=0
    ):  # spacing is a fraction
        assert size != 0.0
        assert spacing != 0.0
        self.rows = rows
    ...
    def is_detection_valid(self, detection, image):
        ...
        if not d.good:
            return False
        if not (
            self.shift <= d.id < self.rows * self.columns + self.shift
        ):  # original code divides this by 4????
            # print(self.shift, d.id, self.rows*self.columns)
            return False
    ...
    def get_tag_corners_for_id(self, tag_id):
        ...
        a = (
            self.size
        )  # https://user-images.githubusercontent.com/5337083/41458381-be379c6e-7086-11e8-9291-352445140e88.png
        b = self.spacing * a
